    ...
    // Isolate a stacking context on the mount so mix-blend-mode on overlay
    // content (e.g. a logo set to "difference") blends against the swarm and
    // nothing behind the hero.
    ...
    // Canvas is the background layer (first child, no z-index → does not form an
    // isolating stacking context); clicks pass through to the content above it.
    renderer.domElement.style.cssText = "position:absolute;inset:0;width:100%;height:100%;display:block;pointer-events:none";
    // Lift existing hero content above the swarm via position ONLY — NOT z-index,
    // which would create a stacking context and break blending against the canvas.
    if (!createdMount) {
    ...
    // NDC is computed against the canvas box (not the window) so the cursor
    // maps correctly when the swarm is embedded in a page section.
